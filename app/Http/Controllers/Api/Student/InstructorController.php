...
class InstructorController extends Controller
{
    ...
    public function profile($id): \Illuminate\Http\JsonResponse
    {
        try {
            ...
            $total_student = User::whereHas('checkout', function ($query) use ($total_course) {
                $query->whereHas('enrolls', function ($query) use ($total_course) {
                    $query->whereIn('enrollable_id', $total_course)->where('enrollable_type', Course::class);
                });
            })->count();
            $social_links  = [];
