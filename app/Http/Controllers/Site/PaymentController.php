...
{
    use PaymentTrait, ApiReturnFormatTrait, SendNotification;
    ...
    public function findAmount($data, $orders = null, $active_currency = null)
    {
        ...
        if (arrayCheck('type', $data) && $data['type'] == 'wallet') {
            ...
            if ($active_currency) {
                $amount = ($orders->sum('total_amount') - ($orders->sum('discount') + $coupon_discount)) * $active_currency->exchange_rate;
            } else {
                $amount = $orders->sum('total_amount') - ($orders->sum('discount') + $coupon_discount);
            }
        }
        return $amount;
    }
