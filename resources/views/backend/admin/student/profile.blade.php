                        ...
                        <div class="col-xxl-9 col-lg-8 col-md-12">
                            <div class="default-tab-list default-tab-list-v2 bg-white redious-border activeItem-bd-none p-20 p-sm-30">
                                <ul class="nav pb-12 mb-20" id="pills-tab" role="tablist">
                                    ...
                                    <li class="nav-item" role="presentation">
                                        <a href="{{ route('students.certificates', $user->id) }}"
                                           class="nav-link {{ request()->route()->getName() == 'students.certificates' ? 'active' : '' }}"
                                           id="certificates">{{ __('certificates') }}</a>
                                    </li>
{{--                                    <li class="nav-item" role="presentation">--}}
{{--                                        <a href="{{ route('students.instructors', $user->id) }}" class="nav-link {{ request()->route()->getName() == 'students.instructors' ? 'active' : '' }}"--}}
{{--                                           id="instructors">{{ __('following_instructor') }}</a>--}}
{{--                                    </li>--}}
                                    <li class="nav-item" role="presentation">
                                        <a href="{{ route('students.payments', $user->id) }}" class="nav-link {{ request()->route()->getName() == 'students.payments' ? 'active' : '' }}"
                                           id="payments">{{ __('payment_history') }}</a>
                                    </li>
                                    <li class="nav-item" role="presentation">
                                        <a href="{{ route('students.activity.logs', $user->id) }}" class="nav-link {{ request()->route()->getName() == 'students.activity.logs' ? 'active' : '' }}"
                                           id="activity_logs">{{ __('login_history') }}</a>
                                    </li>
                                </ul>
                                <!-- End Instructor Profile Tab -->
                                <div class="tab-content" id="pills-tabContent">
                                    @if(request()->route()->getName() == 'students.show')
                                            ...
                                            <div class="row">
                                                <div class="col-md-12">
                                                    <div class="row course_section">
                                                        @include('backend.admin.course.component_with_progress',['progress_bar' => true])
                                                    </div>
                                                </div>
                                    ...
                                    @endif
                                    <!-- END Courses Tab====== -->
                                    @if(request()->route()->getName() == 'students.certificates')
                                            ...
                                            <div class="row">
                                                <div class="col-lg-12">
                                                    <div class="row certificate_section">
                                                        @include('backend.admin.certificate.component')
                                                    </div>
                                                    @if($courses->nextPageUrl())
                                                        <div class="text-center d-block m-t-10">
                                                            <a href="javascript:void(0)"
                                    ...
                                    @endif
{{--                                    @if(request()->route()->getName() == 'students.instructors')--}}
...
{{--                                        </div>--}}
{{--                                    @endif--}}
                                    @if(request()->route()->getName() == 'students.payments')
...
@endsection
@push('js')
    @if(request()->route()->getName() == 'students.activity.logs' || request()->route()->getName() == 'students.payments' || request()->route()->getName() == 'students.instructors')
        {!! $dataTable->scripts() !!}
    @endif
    <script>
        ...
        $(document).ready(function () {
            $(document).on('click', '.load_more_course', function () {
                ...
                $.ajax({
                    url: "{{ route('students.load.course') }}",
                    type: "POST",
                    data: form,
            ...
            $(document).on('click', '.load_more_book', function () {
                ...
                $.ajax({
                    url: "{{ route('students.load.certificates') }}",
                    type: "POST",
                    data: form,
                    success: function (data) {
