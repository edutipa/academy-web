                                ...
                                <div class="col-lg-12">
                                    <div class="course-details-info">
                                        <div class="course-video">
                                            @if($course->video_source == 'mp4' || $course->video_source == 'upload')
                                                <video class="course-intro-video" id="player" playsinline controls
                                                       data-poster="{{ getFileLink('402x248', $course->image) }}">
                                                    <source
                                                        src="{{ $course->video_source == 'upload' ? get_media(getArrayValue('image',$course->video),getArrayValue('storage',$course->video)) : $course->video }}"
                                                        type="video/mp4"/>
                                                </video>
                                            @else
                                                <div class="course-intro-video"
                                                     data-poster="{{ getFileLink('402x248', $course->image) }}"
                                                     data-plyr-provider="{{ $course->video_source }}"
                                                     data-plyr-embed-id="{{ $course->video }}"
                                                ></div>
                                            @endif
                                        </div>
                                            ...
                                            @if ($course->is_discount == 1)
                                                <p class="course-countdown m-b-15">
                                                    <svg width="16" height="16" viewBox="0 0 16 16" fill="none"
                                                        ...
                                                        {{ getRemainingDaysHours($course->discount_end_at)['hours'] }} hours left at this price!</span>
                                            ...
                                            <div class="d-flex gap-3 justify-content-between">
                                                @if (!auth()->check() || auth()->user()->user_type == 'student')
                                                    ...
                                                    @elseif($course->is_discountable == 1)
                                                        <span class="price">
                                                            {{ get_price($course->discount_amount) }}
                                                            <small>{{ get_price($course->price) }}</small>
                                                        </span>
                                                    @elseif ($course->is_renewable)
                                                        <span class="price">
                                                        {{ get_price($course->renew_price) }}
                                                    </span>
                                                    @else
                                                        <span class="price">
                                                            {{ get_price($course->price) }}
                                                        </span>
                                                    @endif
                                                @else
                                                    @if($course->is_free == 1 || $course->price == 0)
